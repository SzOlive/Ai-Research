...
Loss_real = -log(D(real_image))

For fake images: Say FAKE (0)

Loss_fake = -log(1 - D(fake_image)) 
...
Real Image ──► D ──► [0.7] (fairly confident)
...
- **Discriminator (D)** wants to maximize: 
  - log D(x) [high for real images]
  - log(1 - D(G(z))) [high when it correctly rejects fakes]
